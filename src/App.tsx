...
function App() {
  ...
  const onAutoSort = () => {
    const result = groupDeck(playerDecks.p1);
    setPlayerDecks((cr) => ({ ...cr, p1: result }));
  };
    ...
    const checkCases = () => {
      ...
      if (checkIsRequiredTile(deckWithThrownTile, currentPoint)) {
        ...
        if (checkIsWinning(deckWithThrownTile)) {
          ...
          botDiscard(
            deckWithThrownTile[deckWithThrownTile.length - 1],
            playing,
            deckWithThrownTile
          );
        }
        ...
        if (checkIsWinning(updatedDeck)) {
          ...
          botDiscard(updatedDeck[updatedDeck.length - 1], playing, updatedDeck);
        }
      }
    };
    checkCases();
  ...
  const checkIsWinning = (deck: any) => {
    if (deck.length !== 15) {
      console.error("Deck length is not equals to 15");
      return false;
    }
          ...
          <div className="board-container">
            <div className="tile-container">
              {Array.from({ length: 26 })?.map((_, index: number) => {
                const tile = playerDecks?.p1?.find(
                  (tile) => tile.column === index + 1
                );
                return (
                  <div
                    key={index}
